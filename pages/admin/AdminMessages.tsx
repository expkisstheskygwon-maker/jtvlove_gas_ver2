...
const AdminMessages: React.FC = () => {
    const { user } = useAuth();
    const venueId = (user as any)?.venueId || localStorage.getItem('selectedVenueId') || '';
    const venueName = (user as any)?.venueName || localStorage.getItem('selectedVenueName') || '';
    ...
    const fetchMessages = async () => {
        ...
        try {
            if (tab === 'inbox') {
                const data = await apiService.getMessages({ receiverId: venueId, receiverType: 'venue_admin' });
                setMessages(data);
            } else {
                const data = await apiService.getMessages({ senderId: venueId, senderType: 'venue_admin' });
                setMessages(data);
            }
        } catch (e) {
            console.error(e);
