...
export const onRequestGet = async (context: { request: Request; env: Env }) => {
  ...
  try {
    const obj: any = await env.R2.get(decodeURIComponent(key));
    if (!obj || !obj.body) {
    ...
    return new Response(JSON.stringify({ error: error.message || 'R2 proxy error' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
};
